...
// Basically this void function that I have created is to display a farewell
// message to the user
void exitProgram() { // The function main begins
  printf("Thanks for using my Simple Calculator. Hope to see you again soon. "
         "Goodbye! "); // this is entred once the user has exit the program by
                       // selecting option X
}
...
int unOperationIsValid(char operation) {
  if (operation == 'S' || operation == 'L' || operation == 'E' ||
...
void calculateForU(double num, char operation) {
  ...
  case 'F':
    ...
    break;
  default:
    puts("Invalid input for operations. You have to choose from (S, L, E, C, "
         "F).");
    break;
  }
}
void UnaryMO() {
  ...
  while (!unOperationIsValid(operation)) {
    printf("Invalid input! Please select an operation from here (S, L, E, "
           "C, F):\n");
    scanf(" %c", &operation);
  }
...
int main() {
  ...
  while (loop == 1) {
    ...
    if (option == 'B') {
      ...
      char childOption;
      printf("Please select an option B, U or E:\n");
      scanf(" %c", &childOption);
      ...
      case 'B': {
        ...
        char isNumOrVar;
        printf("Is the first number a Variable or a Number? Enter V for "
               "variable and N for number:\n");
        scanf(" %c", &isNumOrVar);
        ...
        printf("Is the second number a Variable or a Number? Enter V for "
               "variable and N for number:\n");
        scanf(" %c", &isNumOrVar);
      ...
      case 'U': {
        ...
        printf("Is the number a Variable or a Number? Enter V for variable and "
               "N for number:\n");
        scanf(" %c", &isNumOrVar);
      ...
      default:
        ...
        printf("Invalid selection. Exited \n");
        break;
      }
    }

    if (loop != 0) {
      printf("\nTo begin another calculation, please select your option (B, U, "
             "A, V, E):\n");
      scanf(" %c", &option);
    }
  }

  exitProgram(); // Once the program has exited the program it will call the
                 // exitProgram function and state a farewell message that had
                 // been assigned at the very begining
